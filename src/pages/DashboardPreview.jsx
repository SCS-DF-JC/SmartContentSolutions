...
export default function DashboardPreview() {
  ...
  // Check for just subscribed flow
  useEffect(() => {
    const checkOnboardingFlow = async () => {
      const justSubscribed = sessionStorage.getItem("just_subscribed");
      
      if (justSubscribed && isAuthenticated) {
        sessionStorage.removeItem("just_subscribed");
        
        // Check if user needs to see welcome modal
        if (user && isSubscriptionActive() && !user.has_seen_welcome) {
          setShowWelcomeModal(true);
        }
      } else if (isAuthenticated && user && isSubscriptionActive() && !user.has_seen_welcome) {
        // User has active subscription but hasn't seen welcome
        setShowWelcomeModal(true);
      }
    };
